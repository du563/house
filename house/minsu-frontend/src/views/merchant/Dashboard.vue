  ...
  <div class="dashboard-page">
    <el-card class="hero-card">
      <div class="hero-main">
        <div>
          <h2>商户运营看板</h2>
          <p>关注订单转化、热门房源与审核进展，提升民宿经营效率。</p>
        </div>
        <div class="hero-actions">
          <el-button type="primary" @click="$router.push('/merchant/house')">我的房源</el-button>
          <el-button @click="$router.push('/merchant/order')">订单管理</el-button>
          <el-button @click="$router.push('/merchant/comment')">评论回复</el-button>
          <el-select
    ...
    <div class="kpi-grid">
      <el-card class="kpi-card">
        ...
        <div><div class="kpi-value">{{ stats.totalCount || 0 }}</div><div class="kpi-label">我的房源</div></div>
      </el-card>
      <el-card class="kpi-card">
        <div class="kpi-icon warm"><i class="el-icon-time"></i></div>
        <div><div class="kpi-value">{{ stats.pendingCount || 0 }}</div><div class="kpi-label">待审核</div></div>
      </el-card>
      <el-card class="kpi-card">
        <div class="kpi-icon green"><i class="el-icon-circle-check"></i></div>
        <div><div class="kpi-value">{{ stats.approvedCount || 0 }}</div><div class="kpi-label">已通过</div></div>
      </el-card>
      <el-card class="kpi-card">
        <div class="kpi-icon rose"><i class="el-icon-warning-outline"></i></div>
        <div><div class="kpi-value">{{ stats.rejectedCount || 0 }}</div><div class="kpi-label">已驳回</div></div>
      </el-card>
    </div>
    ...
    <el-card class="funnel-card">
      <div slot="header" class="card-head">
        ...
        <span class="muted">浏览 → 下单 → 支付 → 完成</span>
      </div>
...
import { getMerchantHouseStats } from '@/api/house'
import { merchantOrderAnalytics } from '@/api/analytics'
...
export default {
  ...
  methods: {
    ...
    async loadStats () {
      try {
        const res = await getMerchantHouseStats()
        this.stats = res.data || {}
      } catch (e) {
        console.error(e)
      }
    ...
    async loadAnalytics () {
      try {
        const res = await merchantOrderAnalytics(this.selectedDays)
...
.kpi-icon.green { background: #67c23a; }
.kpi-icon.rose { background: #e58b8b; }
