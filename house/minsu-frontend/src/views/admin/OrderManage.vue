      ...
      <div class="filter-area">
        <el-form :inline="true">
          <el-form-item label="订单号">
            <el-input v-model="searchOrderNo" placeholder="请输入订单号" clearable></el-input>
          </el-form-item>
      ...
      <el-table :data="orderList" v-loading="loading" border>
        ...
        <el-table-column label="操作" width="200" fixed="right">
          <template slot-scope="scope">
            ...
            <el-button type="text" @click="handleComplete(scope.row)" v-if="scope.row.status === 1">完成</el-button>
          </template>
        </el-table-column>
      </el-table>
...
<script>
import { adminGetOrders, completeOrder } from '@/api/order'
...
export default {
  ...
  methods: {
    ...
    handleComplete(row) {
      this.$confirm('确认将该订单标记为已完成吗？', '确认', { type: 'warning' }).then(async () => {
        try {
    ...
    async handleExportExcel () {
      ...
      try {
        ...
        exportOrdersToExcel(all, { filenameBase: '管理员订单', statusText })
